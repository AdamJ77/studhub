...
from studhub.app.utils.logging import ColorFormatter, request_msg_format


class TestColorFormatter:
    @pytest.mark.parametrize(
        "log_level, color, expected_output",
        [
    ...
    @mock.patch("studhub.app.utils.logging.click.style", side_effect=click.style)
    def test_level_name_colors(
        self, mocked_click_style, log_level, color, expected_output
    ):
        dummy_input = "hello"
    ...
    @pytest.mark.parametrize(
        "levelno, expected_log_format",
        [
            (
                0,
                "%(asctime)-22.19s \x1b[1m%(name)-21s\x1b[0m [\x1b[34m%(levelname)s\x1b[0m]:    %(message)s    (%(filename)s:%(lineno)d)",
            ),
            (
                10,
                "%(asctime)-22.19s \x1b[1m%(name)-21s\x1b[0m [\x1b[36m%(levelname)s\x1b[0m]:    %(message)s    (%(filename)s:%(lineno)d)",
            ),
            (
                20,
                "%(asctime)-22.19s \x1b[1m%(name)-21s\x1b[0m [\x1b[32m%(levelname)s\x1b[0m]:    %(message)s    (%(filename)s:%(lineno)d)",
            ),
            (
                30,
                "%(asctime)-22.19s \x1b[1m%(name)-21s\x1b[0m [\x1b[33m%(levelname)s\x1b[0m]:    %(message)s    (%(filename)s:%(lineno)d)",
            ),
            (
                40,
                "%(asctime)-22.19s \x1b[1m%(name)-21s\x1b[0m [\x1b[31m%(levelname)s\x1b[0m]:    %(message)s    (%(filename)s:%(lineno)d)",
            ),
            (
                50,
                "%(asctime)-22.19s \x1b[1m%(name)-21s\x1b[0m [\x1b[91m%(levelname)s\x1b[0m]:    %(message)s    (%(filename)s:%(lineno)d)",
            ),
        ],
    )
    ...
    def test_format_request_msg(self, status, expected_status):
        ...
        result = ColorFormatter().format_request_msg(
    ...
    @mock.patch("studhub.app.utils.logging.logging.Formatter")
    def test_format(self, mocked_formatter, level_no, level_name, color_fmt):
        ...
        record = mock.MagicMock(
            ...
            msg=request_msg_format,
            args=("0.0.0.0", 80, "GET /dummy/path HTTP/1.1", "200 OK", 0.32),
            levelno=level_no,
        ...
        log_fmt = (
            f"%(asctime)-22.19s \x1b[1m%(name)-21s\x1b[0m [\x1b[{color_fmt}"
