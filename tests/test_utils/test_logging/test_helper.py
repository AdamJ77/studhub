...
from studhub.app.utils.logging import get_request_msg_args, status_code_colors


@pytest.mark.parametrize(
    "key, color, expected_output",
    [
...
@pytest.mark.parametrize(
    "response_status_code, expected_status",
    [
        (100, "100 Continue"),
        (200, "200 OK"),
...
def test_get_request_msg_args(response_status_code, expected_status):
    expected_result_format = (
        '0.0.0.0:80 - "\x1b[1mGET /dummy/path ' 'HTTP/1.1\x1b[0m" {} - 0.32ms'
    )
    ...
    host, port, method_path, status, process_time = get_request_msg_args(
        request, response, 0.32
    )
    assert host == "0.0.0.0"
    assert port == 80
